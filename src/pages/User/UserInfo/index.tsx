...
import {getLoginUserUsingGet, updateKeyUsingPost} from "@/services/xhapi-backend/userController";
import {userSignUsingPost} from "@/services/xhapi-backend/userInterfaceInfoController";
/**
 * 个人中心
 * @constructor
 */
const Index: React.FC = () => {
  ...
  const [userInfo, setData] = useState<API.LoginUserVO>();
  ...
  const updateKey = async () => {
    // @ts-ignore
    const res = await updateKeyUsingPost({id: userInfo.id, userAccount: userInfo.userAccount});
    if (res.code === 0) {
  ...
  return (
    <PageContainer title="个人中心">
        <Row style={{paddingLeft: 20,justifyContent: 'center', alignItems: 'center'}} gutter={[16, 16]}>
          <Col span={12}>
                ...
                <div className="cardContainer">
                  ...
                  <div className="content">
                    ...
                    <div className="description">
                      ...
                      {/* 用户信息内容 */}
                      <div className="item">
                        <span>公钥: {showKeys ? userInfo?.accessKey || '哈哈' : '******'}</span> {/* 当 showKeys 为 true 时显示公钥 */}
                      </div>
                      <div className="item">
                        <span>私钥: {showKeys ? userInfo?.secretKey || '哈哈' : '******'}</span> {/* 当 showKeys 为 true 时显示私钥 */}
                      </div>
                    </div>
